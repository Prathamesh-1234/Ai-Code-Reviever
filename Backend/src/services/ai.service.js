...
const PROFILE_PREFIXES = {
  general: '',
  security: 'Focus ONLY on security vulnerabilities: injection flaws, authentication issues, data exposure, insecure configurations. Ignore style and performance.',
  performance: 'Focus ONLY on performance: time complexity, memory usage, unnecessary operations, caching opportunities. Ignore style issues.',
  style: 'Focus ONLY on code style, naming conventions, readability, and adherence to language best practices. Ignore functionality issues.',
  beginner: 'Explain every issue in plain English as if the reader is a beginner. Avoid jargon. Be encouraging. Add explanatory comments to the improved version.',
};
...
async function aiService(code, reviewProfile = 'general') {
  const prefix = PROFILE_PREFIXES[reviewProfile] || '';
